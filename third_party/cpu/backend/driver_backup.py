...
def compile_module_from_src(src, name):
    ...
    if cache_path is None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Optional: save temporary source for debugging if env var set
            save_src = os.getenv("TRITON_SAVE_TMPSRC")
            if save_src:
                try:
                    import shutil
                    if os.path.isdir(save_src):
                        dst = os.path.join(save_src, f"{name}.cpp")
                    else:
                        dst = save_src
                    shutil.copyfile(src_path, dst)
                    print(f"[triton] saved temporary source to: {dst}")
                except Exception as e:
                    print(f"[triton] failed to save tmp src: {e}")
