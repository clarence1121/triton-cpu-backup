...
# 格式化處理comonicalize 輸入 "const float*" -> 再依照下面cnoniclize_dict轉換成 "*fp32"
...
# 為每個你寫@triton.jit的function的參數賦予一個KernalParam類別的實例
# fun(x,y,z) 就會為x,y,z分別賦予KernelParam
# 每個 KernelParam 會記錄這個參數的名稱、在函式裡的位置索引 (num)、型別註解（annotation）、是否 const/constexpr、
# 有沒有預設值、還有編譜器要不要針對它做 specialization 的旗標（do_not_specialize、do_not_specialize_on_alignment）等等。
...
# 函數工廠 每次呼叫都會回傳一個specialize_impl 函數
# 每次呼叫我們只要給不同的specialize_extra 函數就可以得到不同的 specialize_impl 函數
...
"""
    ...
    # func_namespace是一個dict裡面是string->function
    # 這個函式會被JITFunction.run()呼叫
...
class JITFunction(KernelInterface[T]):
    ...
    def create_binder(self):
        ...
        # 將編譯後的 Kernel 類別綁定到實例上，以便後續使用
        self.CompiledKernel = CompiledKernel
        # 綁定到實體 用於觸發實際編譯流程
        self.compile = compile
        # 將 ASTSource 類別綁定到實例上，用於將 Python 原始碼轉成 AST
        self.ASTSource = ASTSource

        # 根據使用者在 decorator 中定義的函式簽名與參數，
        # 再加上剛剛建立的 backend，產生一個「binder」函數
...
# @overload from typing 只是用來讓ide檢查（擴大允許範圍）不會影響實作
# 在runtime 的時候所有被overload裝飾的function都會被忽略

# 如果是用@jit 編譯器會自動把你裝飾的function當作為一個參數傳入 也就是第一個用法
# 如果用的是@jit() 此時傳入funciton 為none
...
# Optional[x,y,z]帶表x,y,z三種型別都可以接收
# Callable 代表所有有實作__call__的物件
# T是來自typing 模組的泛型類別，代表一個可呼叫的物件
@overload
def jit(
    *,
    version=None,
